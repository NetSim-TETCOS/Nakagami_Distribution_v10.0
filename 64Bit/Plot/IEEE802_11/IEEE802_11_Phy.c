...
int fn_NetSim_IEEE802_11_PhyOut()
{
	...
	list->pstruPhyData->dEndTime = time;
...
int fn_NetSim_IEEE802_11_PhyIn()
{
	...
	ifids = get_IEEE802_11_Interface(packet->nTransmitterId,&i);
	ifid=ifids[0];
	free(ifids);
	...
	/*dFadingPower = propagation_calculate_fadingloss(propagationHandle,
													...
													pstruEventDetails->nDeviceId,
													pstruEventDetails->nInterfaceId);*/
	dFadingPower = fn_netsim_matlab_run("nakagami");
...
bool fn_NetSim_IEEE802_11_TransmitFrame(NetSim_PACKET* pstruPacket, NETSIM_ID nDevId, NETSIM_ID nInterfaceId,double time)
{
	...
	{
		if(GET_RX_POWER_dbm(nDevId,destId,time) >= srcPhy->dCurrentRxSensitivity_dbm)
		{
...
*/
int fn_NetSim_IEEE802_11_SetDataRate(NETSIM_ID nDeviceId, NETSIM_ID nInterfaceId, NETSIM_ID nReceiverId,NetSim_PACKET* packet,double time)
{
	...
	{
	case IEEE_802_11b:
		fn_NetSim_IEEE802_11_DSSSPhy_DataRate(nDeviceId,nInterfaceId,nReceiverId,packet,time);	
		break;
	case IEEE_802_11a:
	case IEEE_802_11g:
	case IEEE_802_11p:
		fn_NetSim_IEEE802_11_OFDMPhy_DataRate(nDeviceId,nInterfaceId,nReceiverId,packet,time);
		break;
	case IEEE_802_11n:
	case IEEE_802_11ac:
		fn_NetSim_IEEE802_11_HTPhy_DataRate(nDeviceId, nInterfaceId, nReceiverId,packet,time);
		break;
	default:
