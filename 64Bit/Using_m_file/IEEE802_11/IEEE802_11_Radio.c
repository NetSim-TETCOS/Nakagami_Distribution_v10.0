...
bool set_radio_state(PIEEE802_11_PHY_VAR phy,PHY_TX_STATUS state)
{
...
pauseBackoff.  
*/
int fn_NetSim_IEEE802_11_UpdatePowerduetoTxStart(NETSIM_ID nDeviceId,NETSIM_ID nInterfaceId,double time,double nav)
{
	...
	for(nLoop = 1;nLoop <= NETWORK->nDeviceCount;nLoop++)
	{
		...
		{
			if(!isIEEE802_11_Configure(nLoop,nLoopInterface))
				continue;
			...
			pstruPhy->dReceivedPower_mw += GET_RX_POWER_mw(nDeviceId,nLoop,time);
			pstruPhy->dReceivedPower_dbm = MW_TO_DBM(pstruPhy->dReceivedPower_mw);
			pstruMac->dNAV = max(pstruMac->dNAV, nav);
...
int	fn_NetSim_IEEE802_11_UpdatePowerduetoTxEnd(NETSIM_ID nDeviceId,NETSIM_ID nInterfaceId)
{
	...
	for(nLoop = 1;nLoop <= NETWORK->nDeviceCount;nLoop++)
	{
		for(nLoopInterface = 1;nLoopInterface<= DEVICE(nLoop)->nNumOfInterface;nLoopInterface++)
		{
			...
			pstruPhy->dReceivedPower_mw -= GET_RX_POWER_mw(nDeviceId,nLoop,p->pstruPhyData->dArrivalTime);
			pstruPhy->dReceivedPower_dbm = MW_TO_DBM(pstruPhy->dReceivedPower_mw);
