...
NETSIM_ID* get_IEEE802_11_Interface(NETSIM_ID nDeviceId,UINT* count)
{
	UINT c=0,c1, c2;
	NETSIM_ID* i1 = fn_NetSim_Stack_GetInterfaceIdbyMacProtocol(nDeviceId, MAC_PROTOCOL_IEEE802_11, &c1);
	NETSIM_ID* i2 = fn_NetSim_Stack_GetInterfaceIdbyMacProtocol(nDeviceId, MAC_PROTOCOL_IEEE1609, &c2);
	NETSIM_ID* i3 = (NETSIM_ID*)calloc(c1 + c2, sizeof* i3);
	NETSIM_ID i;
	for (i = 0; i < c1; i++)
	{
		if (isIEEE802_11_Configure(nDeviceId, i1[i]))
		{
			i3[c] = i1[i];
			c++;
		}
	...
	for (i = 0; i < c2; i++)
	{
		if (isIEEE802_11_Configure(nDeviceId, i2[i]))
		{
			i3[c] = i2[i];
			c++;
		}
