...
double calculate_nav(NETSIM_ID d, NETSIM_ID i, NetSim_PACKET* packet)
{
	...
	case WLAN_RTS:
	{
		double d = (double)(((PIEEE802_11_RTS)(packet->pstruMacData->Packet_MACProtocol))->Duration);
		nav += ceil(packet->pstruPhyData->dStartTime + d) + 3;
	}
	break;
	case WLAN_CTS:
	{
		double d = (double)(((PIEEE802_11_CTS)(packet->pstruMacData->Packet_MACProtocol))->Duration);
		nav += ceil(packet->pstruPhyData->dStartTime + d) + 2;
	}
	break;
	case WLAN_ACK:
